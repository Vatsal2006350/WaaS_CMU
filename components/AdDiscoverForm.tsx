...
import { convertLLMInputToEditorProps } from "./converter";
import { Overlay } from "./converter";
...
export function AdDiscoveryForm() {
  ...
  // Parsed data
  const [parsedData, setParsedData] = useState<Overlay[]>([]);
  ...
  const sendTikToksToAi = async () => {
    ...
    try {
      ...
      const selectedCompanyData = companies.filter((co) =>
        selectedCompanies.includes(co.name)
      );
      // Format the description in the same way as appDescriptions
      const appDescription = selectedCompanyData[0].shortDescription;

      const response = await axios.post(
        "https://221906de1dfae0.lhr.life/build_queries",
        {
          about: appDescription,
          urls: selectedTikToks,
        }
      );

      console.log("AI Processing Response:", response.data);

      // let parseddata = response.data.replace("\\", '"');

      // json parse the parseddata
      // const parsedData2 = JSON.parse(parseddata);

      // TODO: parse
      setParsedData(convertLLMInputToEditorProps(response.data));
    ...
    <>
      {!showEditor ? (
          ...
          >
            ...
            <div className="relative mt-12">
              ...
              <div className="relative bg-gradient-to-r from-purple-50 to-pink-50 rounded-2xl p-6 text-center">
                ...
                {showForm && (
                  <p className="text-gray-600 text-lg">
                    Video previews and analysis will appear here after
                    submission
                  </p>
                )}
              </div>
        ...
        <div className="mt-8 max-w-full">
          ...
          >
            <ReactVideoEditor
              recommendedOverlays={[parsedData]}
              defaultVideos={[]}
            />
          </SidebarProvider>
        </div>
